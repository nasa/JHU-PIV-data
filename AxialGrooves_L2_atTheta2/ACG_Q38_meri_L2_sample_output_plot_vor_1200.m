...
e = pcolor(all_data.norm_Zmesh,all_data.norm_Rmesh,medfilt2(all_data.vor_mean,[1,1]).*normvor);
shading interp
caxis([-100,200]);
axis equal
colormap(jet);
...
% g = patch(x,y,[0,0,1]);
    ...
    g2 = patch(all_data.norm_x_blade1,all_data.norm_y_blade1+0.001,[0.6,0.6,0.6],'EdgeColor','none');
% Add colorbar
...
% Add annotation
text(x_lim(2)+.04,y_lim(1)+(y_lim(2)-y_lim(1))/2-.03,['<\omega_{\theta}>' '/' '\Omega:'],'FontSize',font_size,'FontName',font_name,'FontWeight','bold');
...
%  text(0.26,0.805,'PS','Color','w','FontWeight','bold','FontSize',10);
